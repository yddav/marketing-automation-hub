...
class CustomerDatabase {
  ...
  /**
   * Helper method to run database queries with promises
   */
  ...
  async createTables() {
    try {
      ...
      await this.runQuery(`CREATE TABLE IF NOT EXISTS purchases (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        customer_id INTEGER,
      ...
      await this.runQuery(`CREATE TABLE IF NOT EXISTS support_tickets (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        customer_id INTEGER,
      ...
      await this.runQuery(`CREATE TABLE IF NOT EXISTS communications (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        customer_id INTEGER,
        type TEXT NOT NULL,
        subject TEXT,
      ...
      await this.runQuery(`CREATE INDEX IF NOT EXISTS idx_customers_email ON customers(email)`);
      await this.runQuery(`CREATE INDEX IF NOT EXISTS idx_purchases_sale_id ON purchases(sale_id)`);
      await this.runQuery(`CREATE INDEX IF NOT EXISTS idx_purchases_customer_id ON purchases(customer_id)`);
      await this.runQuery(`CREATE INDEX IF NOT EXISTS idx_support_tickets_customer_id ON support_tickets(customer_id)`);
      await this.runQuery(`CREATE INDEX IF NOT EXISTS idx_communications_customer_id ON communications(customer_id)`);
